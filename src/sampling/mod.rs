...
#[must_use]
pub fn sample(logits: &[f32], config: &SamplingConfig, rng: &mut SimpleRng) -> u32 {
    // CHANGED
...
fn nucleus_filter(mut probs: Vec<f32>, top_p: f32) -> Vec<f32> {
    // CHANGED
...
fn categorical_sample(probs: &[f32], rng: &mut SimpleRng) -> u32 {
    // CHANGED
    let r = rng.next_f32();
    let mut cumsum = 0.0_f32;
...
mod tests {
    ...
    #[test]
    fn test_sample_always_in_vocab() {
        // CHANGED
    ...
    #[test]
    fn test_nucleus_filter_renormalises() {
        // CHANGED
