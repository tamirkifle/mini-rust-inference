...
impl Session {
    ...
    #[must_use]
    pub fn new(model: Arc<LlamaModel>, tokenizer: Arc<Tokenizer>, config: SessionConfig) -> Self {
    ...
    fn decode_loop(&mut self, mut last_logits: Vec<f32>) -> Result<String> {
        ...
        for _ in 0..max_new {
            ...
            let next_id = sample(&last_logits, &self.config.generate.sampling, &mut self.rng);
...
mod tests {
    ...
    fn make_tokenizer() -> Tokenizer {
        use crate::gguf::keys;
        let vocab: Vec<String> = ["<unk>", "<s>", "</s>", "a", "b", "c", "d", "e", "f", "g"]
            .iter()
            .map(|s| s.to_string())
            .collect();
        let scores = vec![
    ...
    fn make_session(max_new_tokens: usize) -> Session {
        ...
        Session::new(
            model,
            tok,
            SessionConfig::new(GenerateConfig::greedy(max_new_tokens), 4, 64),
        )
    }
    ...
    fn test_session_greedy_deterministic() {
        ...
        assert_eq!(
            s1.generate("a").unwrap(),
            s2.generate("a").unwrap(),
    ...
    fn test_session_tokens_used_advances_after_generate() {
        ...
        assert!(
            s.tokens_used() > 0,
            "tokens_used should be > 0 after generate"
        );
    }
    ...
    fn test_session_extend_advances_position_further() {
        ...
        s.extend("b").unwrap();
        assert!(
            s.tokens_used() > pos_after_generate,
    ...
    fn test_session_respects_max_new_tokens() {
        ...
        assert!(
            out.chars().count() <= 2,
            "output must not exceed max_new_tokens=2 chars, got {:?}",
            out
        );
    }
