...
use crate::ops::{matmul::matmul_parallel, norm::rmsnorm};
...
mod tests {
    ...
    fn test_parallel_matches_chunked_single_chunk() {
        ...
        let chunked_logits = ChunkedPrefill::new(tokens.len())
            .run(&model, tokens)
            .unwrap();
        let parallel_logits = ParallelPrefill::new(tokens.len())
            .run(&model, tokens)
            .unwrap();

        assert_eq!(chunked_logits.dims(), parallel_logits.dims());
        for (i, (&c, &p)) in chunked_logits
            .as_slice()
            .iter()
            .zip(parallel_logits.as_slice())
            .enumerate()
        {
            assert!((c - p).abs() < 1e-4, "logit[{i}]: chunked={c} parallel={p}");
        }
    }
