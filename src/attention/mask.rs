...
pub fn causal_mask(seq_len: usize) -> Result<Tensor<f32>> {
    // CHANGED
    if seq_len == 0 {
        // CHANGED
...
pub fn causal_mask_with_offset(
    ...
    for i in 0..seq_q {
        ...
        for j in (abs_pos + 1)..seq_k {
            // CHANGED: mask all keys after this position
...
#[must_use = "returns the masked attention output"] // CHANGED
pub fn masked_sdpa_with_offset(
    // CHANGED
    q: &Tensor<f32>,
    k: &Tensor<f32>,
    v: &Tensor<f32>,
...
mod tests {
    ...
    fn test_causal_mask_1x1() {
        // CHANGED: degenerate single token
    ...
    #[test]
    fn test_causal_mask_offset_shape() {
        // CHANGED
    ...
    #[test]
    fn test_causal_mask_offset_single_token_decode() {
        // CHANGED
    ...
    #[test]
    fn test_causal_mask_offset_matches_causal_mask_when_full() {
        // CHANGED
    ...
    #[test]
    fn test_causal_mask_offset_multi_query() {
        // CHANGED
    ...
    #[test]
    fn test_causal_mask_offset_out_of_range_rejected() {
        // CHANGED
    ...
    #[test]
    fn test_masked_sdpa_future_positions_zero() {
        // CHANGED
        ...
        assert!(
            close(out.as_slice()[0], 10.0),
            "out[0,0] = {}, expected 10.0",
            out.as_slice()[0]
        );
        assert!(
            close(out.as_slice()[1], 20.0),
            "out[0,1] = {}, expected 20.0",
            out.as_slice()[1]
        );
    }

    #[test]
    fn test_masked_sdpa_last_token_sees_all() {
        // CHANGED
        ...
        let v =
            Tensor::from_vec(vec![0.0_f32, 0.0, 1.0, 1.0, 2.0, 2.0, 3.0, 3.0], vec![4, 2]).unwrap();
        ...
        assert!(
            (last[0] - 1.5).abs() < 1e-5,
            "out[3,0] = {}, expected 1.5",
            last[0]
        );
        assert!(
            (last[1] - 1.5).abs() < 1e-5,
            "out[3,1] = {}, expected 1.5",
            last[1]
        );
    }

    #[test]
    fn test_masked_sdpa_seq_mismatch_rejected() {
        // CHANGED
    ...
    #[test]
    fn test_masked_sdpa_with_offset_single_token() {
        // CHANGED
    ...
    #[test]
    fn test_masked_sdpa_with_offset_blocks_future() {
        // CHANGED
        ...
        let v = Tensor::from_vec(
            vec![1.0_f32, 1.0, 1.0, 1.0, 99.0, 99.0], // should be masked
            vec![3, 2],
        )
        .unwrap();
        ...
        assert!(
            (out.as_slice()[0] - 1.0).abs() < 1e-4,
            "out[0] = {} — future token leaked!",
            out.as_slice()[0]
        );
        assert!(
            (out.as_slice()[1] - 1.0).abs() < 1e-4,
            "out[1] = {} — future token leaked!",
            out.as_slice()[1]
        );
    }
}
