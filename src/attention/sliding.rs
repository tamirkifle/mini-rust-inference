...
pub fn sliding_window_gqa(
    ...
    use crate::attention::multihead::{concat_heads, split_head};
...
mod tests {
    ...
    fn test_mask_future_keys_always_masked() {
        ...
        assert!(
            s[1].is_infinite() && s[1] < 0.0,
            "future key 1 must be masked"
        );
        assert!(
            s[2].is_infinite() && s[2] < 0.0,
            "future key 2 must be masked"
        );
        assert!(
            s[3].is_infinite() && s[3] < 0.0,
            "future key 3 must be masked"
        );
        // row 1
        assert!(close(s[4], 0.0));
        assert!(close(s[5], 0.0));
        assert!(
            s[6].is_infinite() && s[6] < 0.0,
            "future key 2 must be masked"
        );
        assert!(
            s[7].is_infinite() && s[7] < 0.0,
            "future key 3 must be masked"
        );
    }
    ...
    fn test_mask_multi_query_sliding() {
        ...
        assert!(close(s[3], 0.0));
        assert!(s[4].is_infinite());
        assert!(s[5].is_infinite());
        // row 1
        assert!(s[6].is_infinite());
        assert!(s[7].is_infinite());
        assert!(s[8].is_infinite());
        assert!(close(s[9], 0.0));
        assert!(close(s[10], 0.0));
        assert!(s[11].is_infinite());
        // row 2
        for j in 0..4 {
            assert!(s[12 + j].is_infinite(), "key {j} should be masked");
        }
        assert!(close(s[16], 0.0));
        assert!(close(s[17], 0.0));
    }
    ...
    fn test_sdpa_window_blocks_old_tokens() {
        ...
        assert!(
            (out.as_slice()[0] - 10.0).abs() < 1e-4,
            "got {} instead of 10.0 — old token leaked",
            out.as_slice()[0]
        );
    }
    ...
    fn test_sdpa_infinite_window_matches_causal() {
        ...
        let v =
            Tensor::from_vec((0..16).map(|i| (16 - i) as f32 * 0.1).collect(), vec![4, 4]).unwrap();
    ...
    fn test_gqa_output_shape() {
        ...
        let q = Tensor::from_vec(
            vec![0.1_f32; seq_q * n_heads * head_dim],
            vec![seq_q, n_heads * head_dim],
        )
        .unwrap();
        let k = Tensor::from_vec(
            vec![0.1_f32; seq_k * n_kv * head_dim],
            vec![seq_k, n_kv * head_dim],
        )
        .unwrap();
        let v = Tensor::from_vec(
            vec![0.1_f32; seq_k * n_kv * head_dim],
    ...
    fn test_gqa_no_nan() {
        ...
        for &x in out.as_slice() {
            assert!(
                ...
                "non-finite in sliding GQA output"
            );
        }
    }
    ...
    fn test_gqa_infinite_window_matches_standard_gqa() {
        ...
        let v = Tensor::from_vec(
            (0..seq_k * n_kv * head_dim)
                .map(|i| (seq_k * n_kv * head_dim - i) as f32 * 0.02)
                .collect(),
            vec![seq_k, n_kv * head_dim],
        )
        .unwrap();
        ...
        let standard =
            grouped_query_attention_causal_with_offset(&q, &k, &v, n_heads, n_kv, start_pos)
                .unwrap();
        let sliding = sliding_window_gqa(&q, &k, &v, n_heads, n_kv, start_pos, usize::MAX).unwrap();
        for (i, (s, g)) in sliding
            .as_slice()
            .iter()
            .zip(standard.as_slice())
            .enumerate()
        {
    ...
    fn test_gqa_window_limits_context() {
        ...
        let (n_heads, n_kv, head_dim) = (2, 2, 4);
        let q = Tensor::from_vec(
            vec![0.0_f32; seq_q * n_heads * head_dim],
            vec![seq_q, n_heads * head_dim],
        )
        .unwrap();
        let k = Tensor::from_vec(
            vec![0.0_f32; seq_k * n_kv * head_dim],
            vec![seq_k, n_kv * head_dim],
        )
        .unwrap();
        // V[i, :] = [i as f32; n_kv * head_dim]
        let v_data: Vec<f32> = (0..seq_k)
            .flat_map(|i| vec![i as f32; n_kv * head_dim])
            .collect();
        ...
        for &v in row0 {
            assert!((v - 2.0).abs() < 1e-4, "row0: got {v}, expected 2.0");
        }
        for &v in row1 {
            assert!((v - 3.0).abs() < 1e-4, "row1: got {v}, expected 3.0");
        }
    }
